<?php
define( 'MQ_SERVER_ADDRr', 'localhost' );
define( 'MQ_SERVER_PORTr', 25575 );
define( 'MQ_SERVER_PASS', '!blowme!' );
define( 'MQ_TIMEOUTr', 3 );
...
function mrcon($cmdr) {
        try {
                ...
                $Rcon->Connect( MQ_SERVER_ADDRr, MQ_SERVER_PORTr, MQ_SERVER_PASS, MQ_TIMEOUTr );
                $Data = $Rcon->Command( $cmdr );
                if ( $Data === false ){throw new MinecraftRconException( "Failed to get command result." );}
                else if ( StrLen( $Data ) == 0 ){throw new MinecraftRconException( "Got command result, but it's empty." );}
//              echo "$Data";
        }
